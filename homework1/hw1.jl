...
function vecvec_to_matrix(vecvec)
	vecvec_to_matrix = zeros(length(vecvec),length(vecvec[1]))
	for i in 1:length(vecvec)
		vecvec_to_matrix[i,:] = vecvec[i]
	end
	return vecvec_to_matrix
end
...
function matrix_to_vecvec(matrix)
	n,m = size(matrix)
	matrix_to_vecvec = [[matrix[i,j] for i in 1:n] for j in 1:m]
	return matrix_to_vecvec
end
